    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        for step_idx, (sigma_tm, sigma_t, gamma) in enumerate(sigmas_and_gammas):
            ...
            # Apply progressive time-varying CDR beta scaling to token_trans_bias
            progressive_bias_applied = False
            original_token_trans_bias = None
            feats = network_condition_kwargs["feats"]
            if (
                steering_args is not None
                and steering_args.get("progressive_steering", False)
                and "progressive_cdr_token_mask" in feats
                and feats["progressive_cdr_token_mask"].any()
            ):
                cdr_mask = feats["progressive_cdr_token_mask"].to(self.device).to(torch.bool)
                beta_max = float(feats["progressive_beta_max"].item())
                schedule_type = int(feats["progressive_beta_schedule"].item())

                if cdr_mask.dim() == 2:
                    cdr_mask = cdr_mask.squeeze(0)

                # Compute time-varying beta based on schedule
                # steering_t goes from 1.0 (start) to 0.0 (end)
                ...
                else:  # step: full beta in phase 1, zero after
            ...
            if progressive_bias_applied and original_token_trans_bias is not None:
