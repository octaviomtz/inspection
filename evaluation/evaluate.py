#!/usr/bin/env python
"""
Evaluation script for Strategy G+ v2 (Progressive Steering) vs baselines.

Computes DockQ, confidence metrics, per-residue metrics, ensemble diversity,
model selection, delta analysis, and generates summary tables + plots.

Usage:
    conda activate dockq2
    python evaluation/evaluate.py --pred_dir predictions_examples --native_dir pdb_minimized

See evaluation/EVALUATION_STRATEGY.md for full details.
"""
...
# Canonical ordering for display and plots
METHOD_ORDER = ["B1", "B2", "B3", "Gv1", "Gv2"]

# Method definitions: folder name and naming pattern
METHOD_DEFS = {
    "B1": {
        "folder": "antigen_cut",
...
def run_dockq(model_path: str, native_path: str) -> dict:
    ...
    # Extract the antibody-antigen interface specifically
...
def select_best_models(
    ...
    for (method, cplx, sub_exp), group in merged.groupby(
        ...
        if not pd.isna(best_oracle_idx):
            ...
            record["oracle_CAPRI"] = classify_capri(oracle_row["GlobalDockQ"])
            for col in [
                "BA_DockQ", "BA_fnat", "BA_iRMSD", "BA_LRMSD",
                "CA_DockQ", "CA_fnat", "CA_iRMSD", "CA_LRMSD",
            ]:
        ...
        if len(conf_vals) > 0:
            ...
            record["conf_CAPRI"] = classify_capri(conf_row["GlobalDockQ"])
...
# ---------------------------------------------------------------------------
# Delta analysis (Section 5.2 of EVALUATION_STRATEGY.md)
# ---------------------------------------------------------------------------
...
def compute_delta_analysis(
    ...
    """Compute per-complex delta DockQ vs reference method.

    Categorizes each complex as improved (delta > 0.05),
    unchanged (|delta| <= 0.05), or regressed (delta < -0.05).
    """
    ref_data = selected_df[selected_df["method"] == reference][
        ["complex", metric]
    ].rename(columns={metric: f"ref_{metric}"})
    ...
    for method in selected_df["method"].unique():
        ...
        method_data = selected_df[selected_df["method"] == method][
            ["complex", metric]
        ].rename(columns={metric: f"method_{metric}"})
        ...
        for _, row in merged.iterrows():
            delta = row[f"method_{metric}"] - row[f"ref_{metric}"]
            if delta > 0.05:
                category = "improved"
            elif delta < -0.05:
                category = "regressed"
            ...
            records.append(
                {
                    "method": method,
                    "complex": row["complex"],
                    f"ref_{metric}": row[f"ref_{metric}"],
                    f"method_{metric}": row[f"method_{metric}"],
                    "delta": delta,
                    "category": category,
                }
            )
...
def _ordered_methods(methods_present: list[str]) -> list[str]:
    """Return methods in canonical display order."""
...
def generate_plots(
    ...
    methods_present = _ordered_methods(selected_df["method"].unique().tolist())
    ...
    # ------------------------------------------------------------------
    # 2. DockQ scatter: Gv2 vs B1
    # ------------------------------------------------------------------
    if "Gv2" in methods_present and "B1" in methods_present:
        gv2 = selected_df[selected_df["method"] == "Gv2"][
            ["complex", "conf_DockQ"]
        ].rename(columns={"conf_DockQ": "Gv2_DockQ"})
        b1 = selected_df[selected_df["method"] == "B1"][
            ["complex", "conf_DockQ"]
        ].rename(columns={"conf_DockQ": "B1_DockQ"})
        scatter_df = pd.merge(b1, gv2, on="complex")

        if len(scatter_df) > 0:
            fig, ax = plt.subplots(figsize=(6, 6))
            ax.scatter(
                scatter_df["B1_DockQ"],
                scatter_df["Gv2_DockQ"],
                s=50,
                c=COLORS["Gv2"],
                edgecolors="black",
                linewidths=0.5,
            ...
            lim = [0, max(1, scatter_df[["B1_DockQ", "Gv2_DockQ"]].max().max() + 0.05)]
            ax.plot(lim, lim, "k--", alpha=0.5, label="y=x")
            ax.set_xlabel("B1 (no steering) DockQ")
            ax.set_ylabel("G+ v2 (progressive steering) DockQ")
            ax.set_title("G+ v2 vs Baseline 1")
            ax.legend()
            ax.set_xlim(lim)
            ax.set_ylim(lim)
            ...
            for _, r in scatter_df.iterrows():
                ax.annotate(
                    r["complex"],
                    (r["B1_DockQ"], r["Gv2_DockQ"]),
                    fontsize=6,
                    alpha=0.7,
                )
            plt.tight_layout()
            plt.savefig(fig_dir / "dockq_scatter_gv2_vs_b1.png", dpi=150)
            plt.close()
            log.info("Saved dockq_scatter_gv2_vs_b1.png")

    # ------------------------------------------------------------------
    # 3. DockQ scatter: Gv2 vs Gv1
    # ------------------------------------------------------------------
    if "Gv2" in methods_present and "Gv1" in methods_present:
        gv2 = selected_df[selected_df["method"] == "Gv2"][
            ["complex", "conf_DockQ"]
        ].rename(columns={"conf_DockQ": "Gv2_DockQ"})
        gv1 = selected_df[selected_df["method"] == "Gv1"][
            ["complex", "conf_DockQ"]
        ].rename(columns={"conf_DockQ": "Gv1_DockQ"})
        scatter_df = pd.merge(gv1, gv2, on="complex")

        if len(scatter_df) > 0:
            fig, ax = plt.subplots(figsize=(6, 6))
            ax.scatter(
                scatter_df["Gv1_DockQ"],
                scatter_df["Gv2_DockQ"],
                s=50,
                c=COLORS["Gv2"],
                edgecolors="black",
            ...
            lim = [0, max(1, scatter_df[["Gv1_DockQ", "Gv2_DockQ"]].max().max() + 0.05)]
            ax.plot(lim, lim, "k--", alpha=0.5, label="y=x")
            ax.set_xlabel("G v1 (constant beta) DockQ")
            ax.set_ylabel("G+ v2 (progressive steering) DockQ")
            ax.set_title("G+ v2 vs G v1")
            ax.legend()
            ax.set_xlim(lim)
            ax.set_ylim(lim)
            ...
            for _, r in scatter_df.iterrows():
                ax.annotate(
                    r["complex"],
                    (r["Gv1_DockQ"], r["Gv2_DockQ"]),
                    fontsize=6,
                    alpha=0.7,
                )
            plt.tight_layout()
            plt.savefig(fig_dir / "dockq_scatter_gv2_vs_gv1.png", dpi=150)
            plt.close()
            log.info("Saved dockq_scatter_gv2_vs_gv1.png")

    # ------------------------------------------------------------------
    # 4. CAPRI classification stacked bar
    # ------------------------------------------------------------------
    ...
    capri_pct = capri_pct.reindex([m for m in METHOD_ORDER if m in capri_pct.index])
    ...
    # ------------------------------------------------------------------
    # 5. Confidence vs DockQ correlation
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 6. Ensemble diversity box plot
    # ------------------------------------------------------------------
    div_valid = diversity_df.dropna(subset=["mean_ca_rmsd"])
    if len(div_valid) > 0:
        methods_in_div = [m for m in METHOD_ORDER if m in div_valid["method"].values]
        data_to_plot = [
    ...
    # ------------------------------------------------------------------
    # 7. Per-complex DockQ comparison (sorted bar chart)
    # ------------------------------------------------------------------
    if len(methods_present) >= 2:
        ...
        if len(pivot) > 0:
            ...
            n_methods = len([m for m in METHOD_ORDER if m in pivot.columns])
            width = 0.8 / max(n_methods, 1)
            for i, method in enumerate(
                [m for m in METHOD_ORDER if m in pivot.columns]
            ):
    ...
    # ------------------------------------------------------------------
    # 8. Delta histogram: Gv2 - B1
    # ------------------------------------------------------------------
    if not delta_df.empty:
        for target_method in ["Gv2", "Gv1"]:
            method_delta = delta_df[delta_df["method"] == target_method]
            if method_delta.empty:
                continue

            fig, ax = plt.subplots(figsize=(7, 5))
            deltas = method_delta["delta"].dropna().values
            n_improved = (deltas > 0.05).sum()
            n_unchanged = ((deltas >= -0.05) & (deltas <= 0.05)).sum()
            n_regressed = (deltas < -0.05).sum()

            ax.hist(deltas, bins=20, color=COLORS.get(target_method, "gray"),
                    edgecolor="black", linewidth=0.5, alpha=0.8)
            ax.axvline(0, color="black", linestyle="--", alpha=0.5)
            ax.axvline(0.05, color="green", linestyle=":", alpha=0.5, label="+0.05")
            ax.axvline(-0.05, color="red", linestyle=":", alpha=0.5, label="-0.05")
            ax.set_xlabel(f"DockQ delta ({target_method} - B1)")
            ax.set_ylabel("Count")
            ax.set_title(
                f"{target_method} vs B1: {n_improved} improved, "
                f"{n_unchanged} unchanged, {n_regressed} regressed"
            )
            ax.legend()
            plt.tight_layout()
            plt.savefig(fig_dir / f"delta_histogram_{target_method.lower()}_vs_b1.png", dpi=150)
            plt.close()
            log.info("Saved delta_histogram_%s_vs_b1.png", target_method.lower())
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate Strategy G+ v2 (Progressive Steering) vs baselines using DockQ"
    )
    parser.add_argument(
        "--pred_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Methods to evaluate (default: all found). Choices: B1 B2 B3 Gv1 Gv2",
    )
    parser.add_argument(
        "--skip_dockq",
        action="store_true",
...
def main():
    ...
    log.info("=" * 60)
    log.info("Step 8: Delta analysis")
    log.info("=" * 60)
    delta_df = pd.DataFrame()
    if "B1" in selected_df["method"].values:
        delta_df = compute_delta_analysis(selected_df, reference="B1")
        delta_df.to_csv(output_dir / "delta_analysis.csv", index=False)

        # Print delta summary per method
        for method in delta_df["method"].unique():
            md = delta_df[delta_df["method"] == method]
            n_imp = (md["category"] == "improved").sum()
            n_unc = (md["category"] == "unchanged").sum()
            n_reg = (md["category"] == "regressed").sum()
            mean_d = md["delta"].mean()
            log.info(
                "  %s vs B1: %d improved, %d unchanged, %d regressed (mean delta=%.4f)",
                method, n_imp, n_unc, n_reg, mean_d,
            )
        print("\nDelta analysis (vs B1):")
        delta_summary = delta_df.groupby("method").agg(
            n_complexes=("complex", "count"),
            n_improved=("category", lambda x: (x == "improved").sum()),
            n_unchanged=("category", lambda x: (x == "unchanged").sum()),
            n_regressed=("category", lambda x: (x == "regressed").sum()),
        ...
        log.info("Skipping delta analysis (B1 not present)")
    ...
    if not args.skip_plots:
        ...
        log.info("=" * 60)
        generate_plots(
            selected_df, dockq_df, confidence_df, diversity_df, delta_df, output_dir
        )
