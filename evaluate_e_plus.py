#!/usr/bin/env python
"""
evaluate_e_plus.py — Evaluation script for E+ (Enhanced Blind Scanning).

Compares E+ against baselines (B1/B2/B3) on three axes:
  Axis A: Epitope prediction accuracy (AUC-ROC, AUC-PR, Precision, Recall, F1, MCC)
  Axis B: Docking / interface quality (DockQ, iRMSD, LRMSD, fnat, ab-aligned ag RMSD)
  Axis C: Confidence metrics (confidence_score, iPTM, pLDDT)

Usage:
  conda run -n boltz python evaluate_e_plus.py --predictions_dir predictions_examples --gt_dir pdb_minimized
"""

import argparse
import json
import logging
import math
import re
import subprocess
import sys
from collections import defaultdict
from pathlib import Path

import numpy as np
import pandas as pd
from Bio.PDB import MMCIFParser, PDBParser

try:
    from scipy.spatial import cKDTree
except ImportError:
    cKDTree = None

try:
    from scipy.stats import spearmanr
except ImportError:
    spearmanr = None

try:
    from sklearn.metrics import average_precision_score, roc_auc_score
except ImportError:
    roc_auc_score = None
    average_precision_score = None

logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")
log = logging.getLogger(__name__)

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------

METHOD_DIRS = {
    "B1": "antigen_cut",
    "B2": "antigen_cut_contact_restraints",
    "B3": "antigen_cut_vhvl_msa_pocket_ab_boltz_post_2023_omm",
    "E+": "new_feature_blind_scanning",
}

DOCKQ_QUALITY = {
    "Incorrect": (0.0, 0.23),
    "Acceptable": (0.23, 0.49),
    "Medium": (0.49, 0.80),
    "High": (0.80, 1.01),
}

ANTIGEN_CHAIN = "A"
HEAVY_CHAIN = "B"
LIGHT_CHAIN = "C"
ANTIBODY_CHAINS = (HEAVY_CHAIN, LIGHT_CHAIN)


# ===========================================================================
# Section 1: Data Discovery
# ===========================================================================


def _extract_complex_name_b1(folder_name):
    """boltz_results_{COMPLEX} -> COMPLEX."""
    m = re.match(r"^boltz_results_(.+)$", folder_name)
    return m.group(1) if m else None


def _extract_complex_name_b2(folder_name):
    """boltz_results_restraint_{COMPLEX}_{TYPE}_{POS} -> COMPLEX, config_label."""
    m = re.match(
        r"^boltz_results_restraint_(.+?)_(hbond|hydrophobic|salt_bridge)_(\d+)$",
        folder_name,
    )
    if m:
        return m.group(1), f"{m.group(2)}_{m.group(3)}"
    return None, None


def _extract_complex_name_b3(folder_name):
    """boltz_results_restraint_to_A_{COMPLEX}_{CHAIN}_{RES}_{POS} -> COMPLEX, config_label."""
    m = re.match(
        r"^boltz_results_restraint_to_A_(.+?)_([A-Z])_([A-Z]+)_(\d+)$",
        folder_name,
    )
    if m:
        return m.group(1), f"{m.group(2)}_{m.group(3)}_{m.group(4)}"
    return None, None


def _extract_complex_name_eplus(folder_name):
    """boltz_results_{COMPLEX}_blind_scan -> COMPLEX."""
    m = re.match(r"^boltz_results_(.+?)_blind_scan$", folder_name)
    return m.group(1) if m else None


def _find_prediction_files(boltz_dir):
    """Find model, confidence, and heatmap files inside a boltz_results_* directory."""
    predictions_dir = boltz_dir / "predictions"
    if not predictions_dir.exists():
        return [], [], []

    model_files = []
    confidence_files = []
    heatmap_files = []
    for subdir in sorted(predictions_dir.iterdir()):
        if not subdir.is_dir():
            continue
        for f in sorted(subdir.iterdir()):
            if f.name.startswith("confidence_") and f.suffix == ".json":
                confidence_files.append(f)
            elif f.name.startswith("epitope_heatmap_") and f.suffix == ".npz":
                heatmap_files.append(f)
            elif f.suffix in (".pdb", ".cif") and "_model_" in f.name:
                model_files.append(f)
    return model_files, confidence_files, heatmap_files


def discover_predictions(predictions_dir):
    """Scan predictions_dir for all methods and their prediction folders.

    Returns: {method: {complex_name: [prediction_info_dict, ...]}}
    """
    predictions_dir = Path(predictions_dir)
    results = {}

    for method, dirname in METHOD_DIRS.items():
        method_dir = predictions_dir / dirname
        if not method_dir.exists():
            log.warning("Method directory not found: %s", method_dir)
            results[method] = {}
            continue

        complexes = defaultdict(list)
        for boltz_dir in sorted(method_dir.iterdir()):
            if not boltz_dir.is_dir() or not boltz_dir.name.startswith("boltz_results"):
                continue

            model_files, confidence_files, heatmap_files = _find_prediction_files(boltz_dir)
            if not model_files:
                log.debug("Skipping %s (no prediction files)", boltz_dir.name)
                continue

            config_label = "default"
            complex_name = None
            if method == "B1":
                complex_name = _extract_complex_name_b1(boltz_dir.name)
            elif method == "B2":
                complex_name, config_label = _extract_complex_name_b2(boltz_dir.name)
            elif method == "B3":
                complex_name, config_label = _extract_complex_name_b3(boltz_dir.name)
            elif method == "E+":
                complex_name = _extract_complex_name_eplus(boltz_dir.name)

            if complex_name is None:
                log.warning("Could not parse complex name from %s", boltz_dir.name)
                continue

            complexes[complex_name].append({
                "folder_path": boltz_dir,
                "complex_name": complex_name,
                "config_label": config_label,
                "model_files": model_files,
                "confidence_files": confidence_files,
                "heatmap_files": heatmap_files,
                "file_format": model_files[0].suffix,
            })

        results[method] = dict(complexes)
        n_complexes = len(complexes)
        n_configs = sum(len(v) for v in complexes.values())
        if n_complexes > 0:
            log.info("  %s: %d complex(es), %d config(s)", method, n_complexes, n_configs)
        else:
            log.info("  %s: no predictions found", method)

    return results


# ===========================================================================
# Section 2: Model Selection
# ===========================================================================


def _load_confidence(json_path):
    """Load a confidence JSON and return its dict."""
    with open(json_path) as f:
        return json.load(f)


def select_best_model(prediction_info):
    """Select the model with the highest confidence_score.

    Returns: (model_file_path, confidence_dict) or (None, None) if no files.
    """
    model_files = prediction_info["model_files"]
    confidence_files = prediction_info["confidence_files"]

    if not model_files:
        return None, None

    model_map = {}
    for mf in model_files:
        m = re.search(r"_model_(\d+)", mf.name)
        if m:
            model_map.setdefault(int(m.group(1)), {})["model"] = mf

    for cf in confidence_files:
        m = re.search(r"_model_(\d+)", cf.name)
        if m:
            idx = int(m.group(1))
            if idx in model_map:
                model_map[idx]["confidence_file"] = cf

    best_score = -1
    best_model = None
    best_conf = None

    for idx in sorted(model_map.keys()):
        entry = model_map[idx]
        if "confidence_file" not in entry:
            continue
        conf = _load_confidence(entry["confidence_file"])
        score = conf.get("confidence_score", 0)
        if score > best_score:
            best_score = score
            best_model = entry["model"]
            best_conf = conf

    if best_model is None and model_files:
        best_model = model_files[0]
        best_conf = {}

    return best_model, best_conf


# ===========================================================================
# Section 3: DockQ Scoring
# ===========================================================================


def _dockq_cache_path(output_dir, method, complex_name, config_label, model_name):
    cache_dir = Path(output_dir) / "dockq_cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir / f"{method}_{complex_name}_{config_label}_{model_name}.json"


def run_dockq(model_path, native_path, output_dir, method, complex_name,
              config_label, skip_dockq=False):
    """Run DockQ and return parsed results dict. Uses caching to avoid re-running."""
    model_name = Path(model_path).stem
    cache_file = _dockq_cache_path(output_dir, method, complex_name, config_label, model_name)

    if cache_file.exists():
        log.debug("Using cached DockQ: %s", cache_file)
        with open(cache_file) as f:
            return json.load(f)

    if skip_dockq:
        log.warning("DockQ skipped (no cache) for %s %s %s", method, complex_name, config_label)
        return None

    dockq_json = cache_file.with_suffix(".dockq_out.json")
    cmd = [
        "conda", "run", "-n", "dockq2",
        "DockQ", str(model_path), str(native_path),
        "--json", str(dockq_json),
    ]

    log.info("Running DockQ: %s vs %s", Path(model_path).name, Path(native_path).name)
    try:
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
    except subprocess.TimeoutExpired:
        log.error("DockQ timed out for %s", model_path)
        return None
    except FileNotFoundError:
        log.error("DockQ command not found. Is dockq2 conda env available?")
        return None

    if proc.returncode != 0:
        log.error("DockQ failed (rc=%d): %s\n%s", proc.returncode, proc.stdout, proc.stderr)
        return None

    if not dockq_json.exists():
        log.error("DockQ JSON output not found at %s", dockq_json)
        return None

    with open(dockq_json) as f:
        raw = json.load(f)

    parsed = _parse_dockq_output(raw)

    with open(cache_file, "w") as f:
        json.dump(parsed, f, indent=2)

    dockq_json.unlink(missing_ok=True)
    return parsed


def _parse_dockq_output(raw):
    """Parse DockQ JSON output into a structured dict."""
    result = {"interfaces": {}, "global_dockq": None}

    best_result = raw.get("best_result", {})

    for interface_key, iface_data in best_result.items():
        if not isinstance(iface_data, dict) or "DockQ" not in iface_data:
            continue
        result["interfaces"][interface_key] = {
            "DockQ": iface_data.get("DockQ"),
            "iRMSD": iface_data.get("iRMSD"),
            "LRMSD": iface_data.get("LRMSD"),
            "fnat": iface_data.get("fnat"),
            "F1": iface_data.get("F1"),
            "fnonnat": iface_data.get("fnonnat"),
        }

    dockq_vals = [v["DockQ"] for v in result["interfaces"].values() if v.get("DockQ") is not None]
    result["global_dockq"] = sum(dockq_vals) / len(dockq_vals) if dockq_vals else None

    ab = result["interfaces"].get("AB", {}).get("DockQ")
    ac = result["interfaces"].get("AC", {}).get("DockQ")
    vals = [v for v in (ab, ac) if v is not None]
    result["ab_ac_avg_dockq"] = sum(vals) / len(vals) if vals else None

    return result


# ===========================================================================
# Section 4: Structure Parsing Helpers
# ===========================================================================


def _load_structure(path):
    """Load a structure using BioPython PDBParser or MMCIFParser."""
    path = Path(path)
    if path.suffix == ".cif":
        parser = MMCIFParser(QUIET=True)
    else:
        parser = PDBParser(QUIET=True)
    return parser.get_structure(path.stem, str(path))


def _is_heavy_atom(atom):
    """Check if atom is a heavy atom (not hydrogen/deuterium)."""
    elem = atom.element.strip().upper()
    if elem in ("H", "D"):
        return False
    name = atom.get_name().strip()
    if not elem and name and name[0] in ("H", "D") and name[0:2] not in ("HE", "HG", "HO"):
        return False
    return True


def _get_chain_atoms(structure, chain_id, heavy_only=True):
    """Get atoms from a specific chain. Returns list of atoms."""
    atoms = []
    for model in structure:
        for chain in model:
            if chain.id == chain_id:
                for residue in chain:
                    if residue.id[0] != " ":
                        continue
                    for atom in residue:
                        if heavy_only and not _is_heavy_atom(atom):
                            continue
                        atoms.append(atom)
        break
    return atoms


def _get_residues(structure, chain_id):
    """Get standard residues from a chain. Returns list of residues."""
    residues = []
    for model in structure:
        for chain in model:
            if chain.id == chain_id:
                for residue in chain:
                    if residue.id[0] != " ":
                        continue
                    residues.append(residue)
        break
    return residues


def _get_ca_coords_ordered(structure, chain_id):
    """Get CA coordinates for a chain in sequential order."""
    coords = []
    for model in structure:
        for chain in model:
            if chain.id != chain_id:
                continue
            for residue in chain:
                if residue.id[0] != " ":
                    continue
                if "CA" in residue:
                    coords.append(residue["CA"].get_vector().get_array())
        break
    return coords


# ===========================================================================
# Section 5: Epitope Extraction
# ===========================================================================


def extract_epitope(structure, cutoff=5.0):
    """Extract epitope residues: antigen residues within cutoff of any antibody heavy atom.

    Returns:
        epitope_set: set of (chain_id, resseq) tuples for epitope residues
        n_antigen: total number of antigen residues
        epitope_binary: list of 0/1 per antigen residue (sequential order)
        min_distances: list of min-distance-to-antibody per antigen residue
    """
    ab_atoms = []
    for chain_id in ANTIBODY_CHAINS:
        ab_atoms.extend(_get_chain_atoms(structure, chain_id, heavy_only=True))

    if not ab_atoms:
        log.warning("No antibody atoms found in structure")
        return set(), 0, [], []

    ab_coords = np.array([a.get_vector().get_array() for a in ab_atoms])

    if cKDTree is not None:
        tree = cKDTree(ab_coords)
    else:
        tree = None

    antigen_residues = _get_residues(structure, ANTIGEN_CHAIN)
    epitope = set()
    epitope_binary = []
    min_distances = []

    for res in antigen_residues:
        res_id = (ANTIGEN_CHAIN, res.id[1])
        heavy_atoms = [a for a in res if _is_heavy_atom(a)]
        if not heavy_atoms:
            epitope_binary.append(0)
            min_distances.append(float("inf"))
            continue

        res_coords = np.array([a.get_vector().get_array() for a in heavy_atoms])

        if tree is not None:
            dists, _ = tree.query(res_coords)
            min_dist = float(dists.min())
        else:
            diff = res_coords[:, np.newaxis, :] - ab_coords[np.newaxis, :, :]
            dists = np.sqrt((diff ** 2).sum(axis=2))
            min_dist = float(dists.min())

        min_distances.append(min_dist)
        if min_dist < cutoff:
            epitope.add(res_id)
            epitope_binary.append(1)
        else:
            epitope_binary.append(0)

    return epitope, len(antigen_residues), epitope_binary, min_distances


# ===========================================================================
# Section 6: Epitope Metrics (Binary)
# ===========================================================================


def compute_epitope_metrics(gt_epitope, pred_epitope, n_antigen_residues):
    """Compute epitope prediction metrics from two sets of (chain, resseq) tuples."""
    tp = len(gt_epitope & pred_epitope)
    fp = len(pred_epitope - gt_epitope)
    fn = len(gt_epitope - pred_epitope)
    tn = max(n_antigen_residues - tp - fp - fn, 0)

    precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0
    recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
    f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0.0

    denom = math.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn))
    mcc = (tp * tn - fp * fn) / denom if denom > 0 else 0.0

    return {
        "TP": tp, "FP": fp, "FN": fn, "TN": tn,
        "Precision": precision, "Recall": recall, "F1": f1, "MCC": mcc,
        "n_gt_epitope": len(gt_epitope), "n_pred_epitope": len(pred_epitope),
        "n_antigen_residues": n_antigen_residues,
    }


def compute_binary_metrics_from_arrays(gt_binary, pred_binary):
    """Compute Precision/Recall/F1/MCC from two aligned binary arrays."""
    gt = np.asarray(gt_binary, dtype=int)
    pred = np.asarray(pred_binary, dtype=int)
    n = min(len(gt), len(pred))
    gt, pred = gt[:n], pred[:n]

    tp = int(((gt == 1) & (pred == 1)).sum())
    fp = int(((gt == 0) & (pred == 1)).sum())
    fn = int(((gt == 1) & (pred == 0)).sum())
    tn = int(((gt == 0) & (pred == 0)).sum())

    precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0
    recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
    f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0.0

    denom = math.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn))
    mcc = (tp * tn - fp * fn) / denom if denom > 0 else 0.0

    return {
        "TP": tp, "FP": fp, "FN": fn, "TN": tn,
        "Precision": precision, "Recall": recall, "F1": f1, "MCC": mcc,
    }


# ===========================================================================
# Section 7: Heatmap-Based Epitope Metrics (E+ specific)
# ===========================================================================


def load_heatmap(heatmap_files):
    """Load epitope heatmap npz file.

    Returns: dict with 'epitope_propensity', 'antigen_token_indices', 'per_region_contacts'
             or None if no heatmap found.
    """
    if not heatmap_files:
        return None

    path = heatmap_files[0]
    try:
        data = np.load(path, allow_pickle=True)
        return {
            "epitope_propensity": data["epitope_propensity"],
            "antigen_token_indices": data["antigen_token_indices"],
            "per_region_contacts": data["per_region_contacts"],
        }
    except Exception as e:
        log.error("Failed to load heatmap %s: %s", path, e)
        return None


def _find_optimal_f1_threshold(gt_binary, scores):
    """Find the threshold that maximizes F1 score."""
    gt = np.asarray(gt_binary, dtype=int)
    scores = np.asarray(scores, dtype=float)

    best_f1 = 0.0
    best_thresh = 0.5

    # Test thresholds from 0.05 to 0.95 in steps of 0.05
    for thresh in np.arange(0.05, 1.0, 0.05):
        pred = (scores >= thresh).astype(int)
        tp = ((gt == 1) & (pred == 1)).sum()
        fp = ((gt == 0) & (pred == 1)).sum()
        fn = ((gt == 1) & (pred == 0)).sum()
        p = tp / (tp + fp) if (tp + fp) > 0 else 0.0
        r = tp / (tp + fn) if (tp + fn) > 0 else 0.0
        f1 = 2 * p * r / (p + r) if (p + r) > 0 else 0.0
        if f1 > best_f1:
            best_f1 = f1
            best_thresh = float(thresh)

    return best_thresh, best_f1


def compute_heatmap_metrics(gt_binary, propensity, min_distances=None):
    """Compute continuous and binary metrics from heatmap propensity vs GT.

    Args:
        gt_binary: list/array of 0/1 per antigen residue (sequential order)
        propensity: array of float propensity values per antigen residue
        min_distances: optional list of min-distance-to-antibody per residue

    Returns: dict of metrics
    """
    gt = np.asarray(gt_binary, dtype=int)
    prop = np.asarray(propensity, dtype=float)

    # Align lengths (antigen residues in GT vs propensity)
    n = min(len(gt), len(prop))
    gt, prop = gt[:n], prop[:n]

    result = {}

    # Continuous metrics
    if roc_auc_score is not None and len(np.unique(gt)) == 2:
        try:
            result["AUC_ROC"] = float(roc_auc_score(gt, prop))
        except ValueError:
            result["AUC_ROC"] = None
    else:
        result["AUC_ROC"] = _compute_auc_roc_manual(gt, prop)

    if average_precision_score is not None and len(np.unique(gt)) == 2:
        try:
            result["AUC_PR"] = float(average_precision_score(gt, prop))
        except ValueError:
            result["AUC_PR"] = None
    else:
        result["AUC_PR"] = _compute_auc_pr_manual(gt, prop)

    # Spearman correlation (propensity vs negative min-distance)
    if min_distances is not None and spearmanr is not None:
        md = np.asarray(min_distances[:n], dtype=float)
        finite_mask = np.isfinite(md)
        if finite_mask.sum() > 3:
            corr, pval = spearmanr(prop[finite_mask], -md[finite_mask])
            result["Spearman_corr"] = float(corr)
            result["Spearman_pval"] = float(pval)
        else:
            result["Spearman_corr"] = None
            result["Spearman_pval"] = None
    else:
        result["Spearman_corr"] = None
        result["Spearman_pval"] = None

    # Optimal F1 threshold
    best_thresh, _ = _find_optimal_f1_threshold(gt, prop)
    result["optimal_threshold"] = best_thresh
    pred_optimal = (prop >= best_thresh).astype(int)
    binary_metrics = compute_binary_metrics_from_arrays(gt, pred_optimal)
    result["Precision"] = binary_metrics["Precision"]
    result["Recall"] = binary_metrics["Recall"]
    result["F1"] = binary_metrics["F1"]
    result["MCC"] = binary_metrics["MCC"]
    result["TP"] = binary_metrics["TP"]
    result["FP"] = binary_metrics["FP"]
    result["FN"] = binary_metrics["FN"]
    result["TN"] = binary_metrics["TN"]

    # Fixed threshold metrics
    for thresh in [0.3, 0.5, 0.7]:
        pred_fixed = (prop >= thresh).astype(int)
        bm = compute_binary_metrics_from_arrays(gt, pred_fixed)
        result[f"F1_t{thresh}"] = bm["F1"]
        result[f"Precision_t{thresh}"] = bm["Precision"]
        result[f"Recall_t{thresh}"] = bm["Recall"]

    result["n_antigen_residues"] = n
    result["n_gt_epitope"] = int(gt.sum())
    result["n_pred_epitope_optimal"] = int(pred_optimal.sum())

    return result


def _compute_auc_roc_manual(gt, scores):
    """Manual AUC-ROC computation when sklearn is unavailable."""
    gt = np.asarray(gt, dtype=int)
    scores = np.asarray(scores, dtype=float)
    if len(np.unique(gt)) < 2:
        return None

    # Sort by decreasing score
    order = np.argsort(-scores)
    gt_sorted = gt[order]

    n_pos = gt.sum()
    n_neg = len(gt) - n_pos
    if n_pos == 0 or n_neg == 0:
        return None

    tpr_prev, fpr_prev = 0.0, 0.0
    auc = 0.0
    tp, fp = 0, 0

    for i in range(len(gt_sorted)):
        if gt_sorted[i] == 1:
            tp += 1
        else:
            fp += 1
        tpr = tp / n_pos
        fpr = fp / n_neg
        auc += (fpr - fpr_prev) * (tpr + tpr_prev) / 2.0
        tpr_prev, fpr_prev = tpr, fpr

    return float(auc)


def _compute_auc_pr_manual(gt, scores):
    """Manual AUC-PR (average precision) when sklearn is unavailable."""
    gt = np.asarray(gt, dtype=int)
    scores = np.asarray(scores, dtype=float)
    if len(np.unique(gt)) < 2:
        return None

    order = np.argsort(-scores)
    gt_sorted = gt[order]

    tp = 0
    ap = 0.0
    n_pos = gt.sum()
    if n_pos == 0:
        return None

    for i in range(len(gt_sorted)):
        if gt_sorted[i] == 1:
            tp += 1
            precision_at_i = tp / (i + 1)
            ap += precision_at_i

    return float(ap / n_pos)


# ===========================================================================
# Section 8: Antibody-Aligned Antigen RMSD
# ===========================================================================


def compute_ab_aligned_ag_rmsd(pred_structure, gt_structure):
    """Compute antigen RMSD after superimposing on antibody CA atoms.

    1. Extract CA atoms from antibody chains (B, C) in both structures
    2. Superimpose prediction onto GT using antibody CAs (matched by sequential position)
    3. Apply transformation to antigen CAs
    4. Compute RMSD of antigen CAs

    Returns: RMSD value (float) or None on failure
    """
    pred_ab_coords_list = []
    gt_ab_coords_list = []
    for chain_id in ANTIBODY_CHAINS:
        pred_cas = _get_ca_coords_ordered(pred_structure, chain_id)
        gt_cas = _get_ca_coords_ordered(gt_structure, chain_id)
        n = min(len(pred_cas), len(gt_cas))
        for i in range(n):
            pred_ab_coords_list.append(pred_cas[i])
            gt_ab_coords_list.append(gt_cas[i])

    if len(pred_ab_coords_list) < 3:
        log.warning("Too few common antibody CA atoms (%d) for superposition",
                     len(pred_ab_coords_list))
        return None

    pred_ab_coords = np.array(pred_ab_coords_list)
    gt_ab_coords = np.array(gt_ab_coords_list)

    pred_ag_cas = _get_ca_coords_ordered(pred_structure, ANTIGEN_CHAIN)
    gt_ag_cas = _get_ca_coords_ordered(gt_structure, ANTIGEN_CHAIN)
    n_ag = min(len(pred_ag_cas), len(gt_ag_cas))
    if n_ag < 1:
        log.warning("No common antigen CA atoms for RMSD")
        return None

    pred_ag_coords = np.array(pred_ag_cas[:n_ag])
    gt_ag_coords = np.array(gt_ag_cas[:n_ag])

    # Kabsch alignment on antibody
    pred_ab_center = pred_ab_coords.mean(axis=0)
    gt_ab_center = gt_ab_coords.mean(axis=0)
    pred_ab_centered = pred_ab_coords - pred_ab_center
    gt_ab_centered = gt_ab_coords - gt_ab_center

    H = pred_ab_centered.T @ gt_ab_centered
    U, S, Vt = np.linalg.svd(H)

    d = np.linalg.det(Vt.T @ U.T)
    sign_matrix = np.diag([1, 1, np.sign(d)])
    rot = Vt.T @ sign_matrix @ U.T

    transformed_ag = (pred_ag_coords - pred_ab_center) @ rot.T + gt_ab_center

    diff = transformed_ag - gt_ag_coords
    rmsd = np.sqrt((diff ** 2).sum(axis=1).mean())
    return float(rmsd)


# ===========================================================================
# Section 9: Confidence Extraction
# ===========================================================================


def extract_confidence(confidence_dict):
    """Extract key confidence metrics from a confidence JSON dict."""
    if not confidence_dict:
        return {}
    return {
        "confidence_score": confidence_dict.get("confidence_score"),
        "iptm": confidence_dict.get("iptm"),
        "ptm": confidence_dict.get("ptm"),
        "complex_plddt": confidence_dict.get("complex_plddt"),
        "complex_iplddt": confidence_dict.get("complex_iplddt"),
    }


# ===========================================================================
# Section 10: Per-Complex Evaluation
# ===========================================================================


def evaluate_complex(method, complex_name, prediction_info, gt_path, output_dir,
                     cutoff, skip_dockq):
    """Evaluate a single method+complex+config. Returns a result dict."""
    model_path, confidence_dict = select_best_model(prediction_info)
    if model_path is None:
        log.warning("No model file for %s / %s / %s", method, complex_name,
                     prediction_info["config_label"])
        return None

    log.info("Evaluating %s / %s [%s] -> %s",
             method, complex_name, prediction_info["config_label"], model_path.name)

    result = {
        "method": method,
        "complex": complex_name,
        "config_label": prediction_info["config_label"],
        "model_file": str(model_path.name),
    }

    # --- Axis B: DockQ ---
    dockq = run_dockq(model_path, gt_path, output_dir, method, complex_name,
                      prediction_info["config_label"], skip_dockq=skip_dockq)
    if dockq:
        result["DockQ_global"] = dockq.get("global_dockq")
        result["DockQ_AB_AC"] = dockq.get("ab_ac_avg_dockq")
        for iface_id in ("AB", "AC", "BC"):
            iface = dockq.get("interfaces", {}).get(iface_id, {})
            if iface:
                result[f"DockQ_{iface_id}"] = iface.get("DockQ")
                result[f"iRMSD_{iface_id}"] = iface.get("iRMSD")
                result[f"LRMSD_{iface_id}"] = iface.get("LRMSD")
                result[f"fnat_{iface_id}"] = iface.get("fnat")
        # Averages for AB+AC
        for metric in ("iRMSD", "fnat"):
            ab_val = result.get(f"{metric}_AB")
            ac_val = result.get(f"{metric}_AC")
            vals = [v for v in (ab_val, ac_val) if v is not None]
            result[f"{metric}_AB_AC"] = sum(vals) / len(vals) if vals else None
    else:
        result["DockQ_global"] = None
        result["DockQ_AB_AC"] = None

    # --- Load structures ---
    try:
        pred_structure = _load_structure(model_path)
        gt_structure = _load_structure(gt_path)
    except Exception as e:
        log.error("Failed to load structures: %s", e)
        return result

    # --- Axis A: Epitope prediction ---
    gt_epitope, n_ag, gt_binary, gt_min_distances = extract_epitope(gt_structure, cutoff=cutoff)

    if method == "E+":
        # Use heatmap if available
        heatmap = load_heatmap(prediction_info.get("heatmap_files", []))
        if heatmap is not None:
            heatmap_metrics = compute_heatmap_metrics(
                gt_binary, heatmap["epitope_propensity"], gt_min_distances
            )
            result.update(heatmap_metrics)
            result["heatmap_available"] = True
        else:
            # Fallback to structure-derived epitope
            log.warning("No heatmap for E+ %s, falling back to structure-derived", complex_name)
            pred_epitope, _, _, _ = extract_epitope(pred_structure, cutoff=cutoff)
            epitope_metrics = compute_epitope_metrics(gt_epitope, pred_epitope, n_ag)
            result.update(epitope_metrics)
            result["heatmap_available"] = False
    else:
        # Baselines: structure-derived epitope
        pred_epitope, _, _, _ = extract_epitope(pred_structure, cutoff=cutoff)
        epitope_metrics = compute_epitope_metrics(gt_epitope, pred_epitope, n_ag)
        result.update(epitope_metrics)
        result["heatmap_available"] = False

    # --- Axis B: Ab-aligned Ag RMSD ---
    ag_rmsd = compute_ab_aligned_ag_rmsd(pred_structure, gt_structure)
    result["ab_aligned_ag_rmsd"] = ag_rmsd

    # --- Confidence ---
    conf = extract_confidence(confidence_dict)
    result.update(conf)

    return result


# ===========================================================================
# Section 11: Aggregation and Best-Per-Complex
# ===========================================================================


def select_best_per_complex(df):
    """For methods with multiple configs (B2, B3), select best config per complex.

    Best = highest confidence_score. Returns a filtered DataFrame.
    """
    if df.empty:
        return df

    rows = []
    for (method, cmplx), group in df.groupby(["method", "complex"]):
        if len(group) == 1:
            rows.append(group.iloc[0])
        else:
            # Select best by confidence_score
            best_idx = group["confidence_score"].idxmax()
            if pd.isna(best_idx):
                best_idx = group.index[0]
            rows.append(group.loc[best_idx])

    return pd.DataFrame(rows).reset_index(drop=True)


def compute_summary(df):
    """Compute summary statistics grouped by method."""
    if df.empty:
        return pd.DataFrame()

    numeric_cols = [
        "DockQ_global", "DockQ_AB_AC", "iRMSD_AB_AC", "fnat_AB_AC",
        "ab_aligned_ag_rmsd",
        "Precision", "Recall", "F1", "MCC",
        "AUC_ROC", "AUC_PR", "Spearman_corr",
        "F1_t0.3", "F1_t0.5", "F1_t0.7",
        "confidence_score", "iptm", "complex_plddt", "complex_iplddt",
    ]
    existing = [c for c in numeric_cols if c in df.columns]

    summary = df.groupby("method")[existing].agg(["mean", "median", "std", "count"])
    return summary


# ===========================================================================
# Section 12: Output — Tables
# ===========================================================================


def _dockq_quality_label(dockq):
    if dockq is None:
        return "N/A"
    for label, (lo, hi) in DOCKQ_QUALITY.items():
        if lo <= dockq < hi:
            return label
    return "N/A"


def _fmt(val, decimals=3):
    if val is None or (isinstance(val, float) and (math.isnan(val) or math.isinf(val))):
        return "—"
    return f"{val:.{decimals}f}"


def print_summary_tables(df, df_best):
    """Print formatted markdown tables to stdout."""
    if df_best.empty:
        print("\nNo results to display.")
        return

    methods = sorted(df_best["method"].unique())

    # Table 1: Epitope Prediction (Axis A)
    print("\n## Table 1: Epitope Prediction (Axis A)")
    print("| Method | AUC-ROC | AUC-PR | F1 (optimal) | MCC | Precision | Recall |")
    print("|--------|---------|--------|--------------|-----|-----------|--------|")
    for m in methods:
        mdf = df_best[df_best["method"] == m]
        row = [m]
        for col in ["AUC_ROC", "AUC_PR", "F1", "MCC", "Precision", "Recall"]:
            if col in mdf.columns:
                val = mdf[col].mean()
                row.append(_fmt(val))
            else:
                row.append("—")
        print(f"| {row[0]:20s} | {row[1]:>7s} | {row[2]:>6s} | {row[3]:>12s} | {row[4]:>5s} | {row[5]:>9s} | {row[6]:>6s} |")

    # Table 1b: E+ fixed-threshold breakdown
    eplus_df = df_best[df_best["method"] == "E+"]
    if not eplus_df.empty and "F1_t0.3" in eplus_df.columns:
        print("\n### E+ Epitope F1 at Fixed Thresholds")
        print("| Threshold | F1 | Precision | Recall |")
        print("|-----------|-----|-----------|--------|")
        for t in [0.3, 0.5, 0.7]:
            f1 = _fmt(eplus_df[f"F1_t{t}"].mean())
            p = _fmt(eplus_df[f"Precision_t{t}"].mean())
            r = _fmt(eplus_df[f"Recall_t{t}"].mean())
            print(f"| {t}         | {f1:>5s} | {p:>9s} | {r:>6s} |")

    # Table 2: Docking Quality (Axis B)
    print("\n## Table 2: Docking Quality (Axis B)")
    dockq_cols = ["DockQ_AB_AC", "DockQ_global", "iRMSD_AB_AC", "fnat_AB_AC", "ab_aligned_ag_rmsd"]
    avail_cols = [c for c in dockq_cols if c in df_best.columns]
    header = "| Method              |" + "|".join(f" {c:>20s} " for c in avail_cols) + "|"
    sep = "|---------------------|" + "|".join("-" * 22 for _ in avail_cols) + "|"
    print(header)
    print(sep)
    for m in methods:
        mdf = df_best[df_best["method"] == m]
        vals = []
        for c in avail_cols:
            vals.append(f" {_fmt(mdf[c].mean()):>20s} ")
        print(f"| {m:20s}|" + "|".join(vals) + "|")

    # Table 3: Confidence Metrics
    print("\n## Table 3: Confidence Metrics")
    conf_cols = ["confidence_score", "iptm", "complex_plddt", "complex_iplddt"]
    avail_conf = [c for c in conf_cols if c in df_best.columns]
    header = "| Method              |" + "|".join(f" {c:>20s} " for c in avail_conf) + "|"
    sep = "|---------------------|" + "|".join("-" * 22 for _ in avail_conf) + "|"
    print(header)
    print(sep)
    for m in methods:
        mdf = df_best[df_best["method"] == m]
        vals = []
        for c in avail_conf:
            vals.append(f" {_fmt(mdf[c].mean(), 4):>20s} ")
        print(f"| {m:20s}|" + "|".join(vals) + "|")

    # Table 4: Per-Complex Breakdown
    print("\n## Table 4: Per-Complex Breakdown")
    print("| Complex         | Method | DockQ(AB+AC) | F1    | AUC-ROC | Quality   |")
    print("|-----------------|--------|--------------|-------|---------|-----------|")
    for _, row in df_best.sort_values(["complex", "method"]).iterrows():
        dq = _fmt(row.get("DockQ_AB_AC"))
        f1 = _fmt(row.get("F1"))
        auc = _fmt(row.get("AUC_ROC"))
        qual = _dockq_quality_label(row.get("DockQ_global"))
        print(f"| {row['complex']:15s} | {row['method']:6s} | {dq:>12s} | {f1:>5s} | {auc:>7s} | {qual:9s} |")


# ===========================================================================
# Section 13: Output — CSV
# ===========================================================================


def save_csv(df, df_best, summary, output_dir):
    """Save results and summary to CSV files."""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    results_csv = output_dir / "results_all_configs.csv"
    df.to_csv(results_csv, index=False)
    log.info("Saved all-config results to %s", results_csv)

    best_csv = output_dir / "results_best_per_complex.csv"
    df_best.to_csv(best_csv, index=False)
    log.info("Saved best-per-complex results to %s", best_csv)

    if not summary.empty:
        summary_csv = output_dir / "summary_by_method.csv"
        summary.to_csv(summary_csv)
        log.info("Saved summary to %s", summary_csv)


# ===========================================================================
# Section 14: Output — Plots
# ===========================================================================


def generate_plots(df_best, df_all, output_dir):
    """Generate evaluation plots. Skips silently if matplotlib is unavailable."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    except ImportError:
        log.info("matplotlib not available, skipping plots")
        return

    plot_dir = Path(output_dir) / "plots"
    plot_dir.mkdir(parents=True, exist_ok=True)
    methods = sorted(df_best["method"].unique())
    colors = ["#4C72B0", "#DD8452", "#55A868", "#C44E52"][:len(methods)]

    # --- Plot 1: Mean DockQ bar chart ---
    if "DockQ_AB_AC" in df_best.columns:
        fig, ax = plt.subplots(figsize=(8, 5))
        means = [df_best[df_best["method"] == m]["DockQ_AB_AC"].mean() for m in methods]
        stds = [df_best[df_best["method"] == m]["DockQ_AB_AC"].std() for m in methods]
        stds = [s if not np.isnan(s) else 0 for s in stds]
        bars = ax.bar(methods, means, yerr=stds, capsize=5, color=colors)
        ax.set_ylabel("DockQ (AB+AC avg)")
        ax.set_title("Docking Quality: DockQ (AB+AC interface)")
        ax.set_ylim(0, 1)
        for bar, val in zip(bars, means):
            if not np.isnan(val):
                ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.02,
                        f"{val:.3f}", ha="center", va="bottom", fontsize=10)
        fig.tight_layout()
        fig.savefig(plot_dir / "dockq_bar.png", dpi=150)
        plt.close(fig)
        log.info("Saved dockq_bar.png")

    # --- Plot 2: Mean epitope F1 bar chart ---
    if "F1" in df_best.columns:
        fig, ax = plt.subplots(figsize=(8, 5))
        means = [df_best[df_best["method"] == m]["F1"].mean() for m in methods]
        stds = [df_best[df_best["method"] == m]["F1"].std() for m in methods]
        stds = [s if not np.isnan(s) else 0 for s in stds]
        bars = ax.bar(methods, means, yerr=stds, capsize=5, color=colors)
        ax.set_ylabel("Epitope F1")
        ax.set_title("Epitope Prediction: F1 Score")
        ax.set_ylim(0, 1)
        for bar, val in zip(bars, means):
            if not np.isnan(val):
                ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.02,
                        f"{val:.3f}", ha="center", va="bottom", fontsize=10)
        fig.tight_layout()
        fig.savefig(plot_dir / "epitope_f1_bar.png", dpi=150)
        plt.close(fig)
        log.info("Saved epitope_f1_bar.png")

    # --- Plot 3: AUC-ROC and AUC-PR bar chart (E+ vs baselines) ---
    for auc_col, auc_name in [("AUC_ROC", "AUC-ROC"), ("AUC_PR", "AUC-PR")]:
        if auc_col in df_best.columns and df_best[auc_col].notna().any():
            fig, ax = plt.subplots(figsize=(8, 5))
            means = []
            stds = []
            for m in methods:
                mdf = df_best[df_best["method"] == m]
                val = mdf[auc_col].mean()
                std = mdf[auc_col].std()
                means.append(val if not np.isnan(val) else 0)
                stds.append(std if not np.isnan(std) else 0)
            bars = ax.bar(methods, means, yerr=stds, capsize=5, color=colors)
            ax.set_ylabel(auc_name)
            ax.set_title(f"Epitope Prediction: {auc_name}")
            ax.set_ylim(0, 1)
            for bar, val in zip(bars, means):
                if val > 0:
                    ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.02,
                            f"{val:.3f}", ha="center", va="bottom", fontsize=10)
            fig.tight_layout()
            fig.savefig(plot_dir / f"{auc_col.lower()}_bar.png", dpi=150)
            plt.close(fig)
            log.info("Saved %s_bar.png", auc_col.lower())

    # --- Plot 4: E+ vs B1 DockQ scatter ---
    if "E+" in methods and "B1" in methods and "DockQ_AB_AC" in df_best.columns:
        b1_df = df_best[df_best["method"] == "B1"].set_index("complex")
        ep_df = df_best[df_best["method"] == "E+"].set_index("complex")
        common = sorted(set(b1_df.index) & set(ep_df.index))
        if common:
            fig, ax = plt.subplots(figsize=(6, 6))
            b1_vals = [b1_df.loc[c, "DockQ_AB_AC"] for c in common]
            ep_vals = [ep_df.loc[c, "DockQ_AB_AC"] for c in common]
            ax.scatter(b1_vals, ep_vals, s=80, zorder=5)
            for c, bv, ev in zip(common, b1_vals, ep_vals):
                ax.annotate(c, (bv, ev), textcoords="offset points",
                            xytext=(5, 5), fontsize=7)
            ax.plot([0, 1], [0, 1], "k--", alpha=0.3, label="y=x")
            ax.set_xlabel("B1 DockQ (AB+AC)")
            ax.set_ylabel("E+ DockQ (AB+AC)")
            ax.set_title("E+ vs B1: Docking Quality")
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1)
            ax.legend()
            fig.tight_layout()
            fig.savefig(plot_dir / "eplus_vs_b1_dockq.png", dpi=150)
            plt.close(fig)
            log.info("Saved eplus_vs_b1_dockq.png")

    # --- Plot 5: E+ vs B1 F1 scatter ---
    if "E+" in methods and "B1" in methods and "F1" in df_best.columns:
        b1_df = df_best[df_best["method"] == "B1"].set_index("complex")
        ep_df = df_best[df_best["method"] == "E+"].set_index("complex")
        common = sorted(set(b1_df.index) & set(ep_df.index))
        if common:
            fig, ax = plt.subplots(figsize=(6, 6))
            b1_vals = [b1_df.loc[c, "F1"] for c in common]
            ep_vals = [ep_df.loc[c, "F1"] for c in common]
            ax.scatter(b1_vals, ep_vals, s=80, zorder=5, color="#C44E52")
            for c, bv, ev in zip(common, b1_vals, ep_vals):
                ax.annotate(c, (bv, ev), textcoords="offset points",
                            xytext=(5, 5), fontsize=7)
            ax.plot([0, 1], [0, 1], "k--", alpha=0.3, label="y=x")
            ax.set_xlabel("B1 Epitope F1")
            ax.set_ylabel("E+ Epitope F1")
            ax.set_title("E+ vs B1: Epitope F1")
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1)
            ax.legend()
            fig.tight_layout()
            fig.savefig(plot_dir / "eplus_vs_b1_f1.png", dpi=150)
            plt.close(fig)
            log.info("Saved eplus_vs_b1_f1.png")

    # --- Plot 6: DockQ quality distribution ---
    if "DockQ_global" in df_best.columns:
        fig, ax = plt.subplots(figsize=(8, 5))
        categories = list(DOCKQ_QUALITY.keys())
        x = np.arange(len(categories))
        width = 0.8 / max(len(methods), 1)

        for i, m in enumerate(methods):
            mdf = df_best[df_best["method"] == m]
            counts = []
            for cat in categories:
                lo, hi = DOCKQ_QUALITY[cat]
                n = ((mdf["DockQ_global"] >= lo) & (mdf["DockQ_global"] < hi)).sum()
                counts.append(n)
            ax.bar(x + i * width, counts, width, label=m, color=colors[i])

        ax.set_xticks(x + width * (len(methods) - 1) / 2)
        ax.set_xticklabels(categories)
        ax.set_ylabel("Count")
        ax.set_title("DockQ Quality Distribution")
        ax.legend()
        fig.tight_layout()
        fig.savefig(plot_dir / "dockq_distribution.png", dpi=150)
        plt.close(fig)
        log.info("Saved dockq_distribution.png")

    # --- Plot 7: Per-complex heatmap visualizations (E+ only) ---
    _plot_heatmaps(df_all, plot_dir)


def _plot_heatmaps(df_all, plot_dir):
    """Generate per-complex propensity heatmap visualizations for E+."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    except ImportError:
        return

    eplus_rows = df_all[(df_all["method"] == "E+") & (df_all.get("heatmap_available", False) == True)]  # noqa: E712
    if eplus_rows.empty:
        return

    heatmap_dir = plot_dir / "heatmaps"
    heatmap_dir.mkdir(parents=True, exist_ok=True)
    log.info("Generating per-complex heatmap plots...")

    for _, row in eplus_rows.iterrows():
        complex_name = row["complex"]

        # We need to reload the heatmap and GT for plotting
        # Store info in the row or re-derive — for now we note this requires
        # the heatmap files which are referenced in the raw prediction_info.
        # The plot generation is best done inline during evaluation.
        # For now, generate a placeholder that references the data.
        log.debug("Heatmap plot for %s (would need heatmap data)", complex_name)


# ===========================================================================
# Section 15: Main CLI
# ===========================================================================


def main():
    parser = argparse.ArgumentParser(
        description="Evaluate E+ (Enhanced Blind Scanning) against baselines")
    parser.add_argument("--predictions_dir", default="predictions_examples",
                        help="Directory containing method subdirectories (default: predictions_examples)")
    parser.add_argument("--gt_dir", default="pdb_minimized",
                        help="Directory containing ground-truth PDB files (default: pdb_minimized)")
    parser.add_argument("--output_dir", default="evaluation_output_eplus",
                        help="Output directory for results (default: evaluation_output_eplus)")
    parser.add_argument("--cutoff", type=float, default=5.0,
                        help="Distance cutoff (A) for epitope definition (default: 5.0)")
    parser.add_argument("--skip_dockq", action="store_true",
                        help="Skip DockQ computation (use cached results only)")
    parser.add_argument("--skip_plots", action="store_true",
                        help="Skip plot generation")
    parser.add_argument("--sensitivity", action="store_true",
                        help="Run sensitivity analysis at cutoffs 4.5, 5.0, 6.0 A")
    parser.add_argument("--verbose", "-v", action="store_true",
                        help="Enable debug logging")
    args = parser.parse_args()

    if args.verbose:
        logging.getLogger().setLevel(logging.DEBUG)

    predictions_dir = Path(args.predictions_dir)
    gt_dir = Path(args.gt_dir)
    output_dir = Path(args.output_dir)

    if not predictions_dir.exists():
        log.error("Predictions directory not found: %s", predictions_dir)
        sys.exit(1)
    if not gt_dir.exists():
        log.error("Ground truth directory not found: %s", gt_dir)
        sys.exit(1)

    output_dir.mkdir(parents=True, exist_ok=True)

    # Build GT lookup
    gt_files = {}
    for f in sorted(gt_dir.glob("*.pdb")):
        gt_files[f.stem] = f
    log.info("Found %d ground-truth structures in %s", len(gt_files), gt_dir)

    # Discover predictions
    log.info("Discovering predictions in %s ...", predictions_dir)
    predictions = discover_predictions(predictions_dir)

    # Determine cutoffs
    cutoffs = [args.cutoff]
    if args.sensitivity:
        cutoffs = [4.5, 5.0, 6.0]

    for cutoff in cutoffs:
        log.info("=" * 60)
        log.info("Evaluating with epitope cutoff = %.1f A", cutoff)
        log.info("=" * 60)

        all_results = []
        for method, complexes in predictions.items():
            for complex_name, configs in complexes.items():
                if complex_name not in gt_files:
                    log.warning("No GT for %s (method %s), skipping", complex_name, method)
                    continue

                gt_path = gt_files[complex_name]
                for pred_info in configs:
                    result = evaluate_complex(
                        method, complex_name, pred_info, gt_path, output_dir,
                        cutoff, args.skip_dockq,
                    )
                    if result:
                        all_results.append(result)

        # Aggregate
        df = pd.DataFrame(all_results)
        if df.empty:
            log.warning("No results collected. Check your predictions directory.")
            continue

        df_best = select_best_per_complex(df)
        summary = compute_summary(df_best)

        # Output
        suffix = f"_cutoff{cutoff}" if len(cutoffs) > 1 else ""

        print(f"\n{'='*60}")
        print(f"Results (cutoff = {cutoff} A)")
        print(f"{'='*60}")
        print_summary_tables(df, df_best)

        cur_output = output_dir if not suffix else output_dir / f"cutoff_{cutoff}"
        save_csv(df, df_best, summary, cur_output)

        if not args.skip_plots:
            generate_plots(df_best, df, cur_output)

    log.info("Evaluation complete. Results in %s", output_dir)


if __name__ == "__main__":
    main()
