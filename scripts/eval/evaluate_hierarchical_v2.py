#!/usr/bin/env python
"""
Evaluation script for Hierarchical Steering v2 (Strategy Y+).

Compares Y+ against 3 baselines across ~48 antibody-antigen complexes.
...
Statistical tests: paired Wilcoxon signed-rank (Y+ vs each baseline),
bootstrap 95% CIs on means.

Usage (dockq2 conda env required):
  /home/oc/anaconda3/envs/dockq2/bin/python scripts/eval/evaluate_hierarchical_v2.py \\
      --ground_truth_dir examples/pdb_minimized \\
      --predictions_dir predictions_examples \\
      --cdrs_csv examples/cdrs.csv \\
      --out_dir evaluation_results_hierarchical_v2

  # Override the folder name used for Y+ predictions (default: hierarchical_steering_v2):
      --hierarchical_v2_subfolder my_custom_folder_name
...
# prefix/suffix: naming convention for the run folder (boltz_results_<prefix><complex><suffix>)
# multi_setting: True if a complex may have multiple run folders (e.g. different restraint types)
METHOD_CONFIGS = {
    "baseline": {
    ...
    # Y+ entry — subfolder can be overridden via --hierarchical_v2_subfolder
    "hierarchical_v2": {
        "subfolder": "hierarchical_steering_v2",
        "prefix": "boltz_results_",
        "suffix": "",
        "multi_setting": False,
    },
...
def find_prediction_files(run_folder):
    ...
    inner = subfolders[0]  # e.g. predictions/7TRH_HBG/
    models = []
...
def evaluate_complex_method(
    ...
    For multi-setting methods (contact_restraints, pocket_guided), each
    run_folder is one setting; we later pick the best.
...
def generate_plots(df, out_dir, active_methods):
    ...
    fig.suptitle("DockQ Metrics — Hierarchical Steering v2 vs Baselines", fontweight="bold")
    plt.tight_layout()
    ...
    # ---- 3. Scatter: Y+ vs baseline ----
    if "hierarchical_v2" in active_methods and "baseline" in active_methods:
        dh = df[df["method"] == "hierarchical_v2"][["complex", "top1_ab_ag_dockq"]].dropna()
        db = df[df["method"] == "baseline"][["complex", "top1_ab_ag_dockq"]].dropna()
        merged = pd.merge(dh, db, on="complex", suffixes=("_v2", "_base"))
        if len(merged) > 0:
            fig, ax = plt.subplots(figsize=(6, 6))
            ax.scatter(
                merged["top1_ab_ag_dockq_base"], merged["top1_ab_ag_dockq_v2"],
                c=METHOD_COLORS["hierarchical_v2"], alpha=0.7, edgecolors="black", linewidths=0.5,
            )
            # Annotate points that improved / degraded noticeably
            for _, row in merged.iterrows():
                diff = row["top1_ab_ag_dockq_v2"] - row["top1_ab_ag_dockq_base"]
                if abs(diff) > 0.15:
                    ax.annotate(row["complex"], (row["top1_ab_ag_dockq_base"], row["top1_ab_ag_dockq_v2"]),
                                fontsize=5, alpha=0.7)
            ax.plot([0, 1], [0, 1], "k--", alpha=0.4, label="y=x")
            ax.set_xlabel("Baseline Ab-Ag DockQ"); ax.set_ylabel("Hierarchical v2 Ab-Ag DockQ")
            ax.set_title("Y+ vs Baseline (per complex)\nAbove diagonal = improvement")
            ax.set_xlim(0, 1); ax.set_ylim(0, 1); ax.set_aspect("equal")
            ax.legend(fontsize=8)
            plt.tight_layout()
            plt.savefig(fig_dir / "scatter_v2_vs_baseline.png", dpi=150)
            plt.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Hierarchical Steering v2 (Strategy Y+) vs baselines",
    ...
    parser.add_argument("--out_dir", default="evaluation_results_hierarchical_v2",
                        help="Output directory (default: evaluation_results_hierarchical_v2)")
    parser.add_argument("--hierarchical_v2_subfolder", default="hierarchical_steering_v2",
                        help="Subfolder name inside predictions_dir for Y+ results "
                             "(default: hierarchical_steering_v2)")
    parser.add_argument("--methods", nargs="+", default=list(METHOD_CONFIGS.keys()),
    ...
    # Apply subfolder override for Y+
    ...
    for _, row in summary_df.iterrows():
        print(
            ...
            f"{row.get('medium_pct', float('nan')) + row.get('high_pct', float('nan')):>8.1f}"
        )

    # ---- Statistical tests ----
    print("\n" + "=" * 90)
    print("STATISTICAL TESTS — hierarchical_v2 vs each baseline (paired Wilcoxon)")
    print("=" * 90)
    print(f"  {'Metric':<28}  {'vs':<22}  {'mean diff':>10}  "
          f"{'W/T/L':>10}  {'p-value':>9}  {'sig':>4}")
    print("  " + "-" * 85)

    test_metrics = [
        "top1_total_dockq", "top1_ab_ag_dockq",
        "cdr_cdr3_h", "epitope_f1_5A", "epitope_f1_8A", "conf_iptm",
    ]
    stat_rows = []
    for baseline_method in [m for m in active_methods if m != "hierarchical_v2"]:
        for metric in test_metrics:
            if metric not in df.columns:
                continue
            res = wilcoxon_test(df, metric, "hierarchical_v2", baseline_method)
            res["comparison"] = f"hierarchical_v2 vs {baseline_method}"
            res["metric"] = metric
            ...
            if res["p_value"] is not None:
                ...
                print(f"  {metric:<28}  vs {baseline_method:<22}  "
